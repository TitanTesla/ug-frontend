...
new Chart(pieCtx, {
    ...
    .addEventListener('click', async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/api/products?name=${encodeURIComponent(
            nameToDelete
  ...
  // === UPDATE PRODUCT ===
  document.getElementById('update-product-btn').addEventListener('click', async e => {
    e.preventDefault();
    ...
    const file = imageInput.files[0]; // Grab image if selected
    ...
    if (!quantity && !price && !file) {
      alert('Please provide either Quantity, Price, or an Image to update.');
      return;
    }
  
    try {
      const formData = new FormData();
      formData.append('quantity', quantity ? Number(quantity) : '');
      formData.append('price', price ? Number(price) : '');
      if (file) formData.append('image', file);
  
      const response = await fetch(
        `${BASE_URL}/api/products?name=${encodeURIComponent(name)}&category=${encodeURIComponent(category)}`,
        {
          method: 'PUT',
          body: formData
        }
